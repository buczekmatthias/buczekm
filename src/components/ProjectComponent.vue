<template>
  <div class="fixed top-0 left-0 w-screen h-screen bg-zinc-950 bg-opacity-70 flex items-center justify-center" id="project">
    <div class="max-w-xl w-full h-screen bg-dark flex flex-col py-4 [&>*]:px-4 sm:h-modal sm:rounded-xl">
      <div class="grid grid-cols-3 border-b-2 border-b-primary border-solid pb-4">
      ...
      <a class="mt-auto text-center border-link-primary border-solid border rounded-md py-4 mx-4 duration-300 hover:-translate-y-2 sticky bottom-0" :href="`https://github.com/buczekmatthias/${project.name}`" target="_blank" rel="noopener noreferrer">Open repositiory</a>
    </div>
  </div>
</template>
